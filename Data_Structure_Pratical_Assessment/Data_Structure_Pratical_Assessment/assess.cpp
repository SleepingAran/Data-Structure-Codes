...
using namespace std;
bool updateDesignation(List *list1, char *old_des, char *new_des);
bool filterByYear(List *list1, int year, List *list2);
...
int menu()
{
	...
	cout << "7. Update Designation" << endl;
	cout << "8. Filter by year" << endl;
	cout << "9. Exit" << endl;
	cout << "Please select the operation to perform (1-9): ";
	cin >> choice;
	return choice;
}
...
int main()
{
	...
	do
	{
		...
		case 7:
			char old_des[30];
			char new_des[30];
			bool check_if_list_is_empty_case_7;
			cout << "Input the old designation: ";
			cin.ignore();
			cin.getline(old_des, 30);
			cout << "Input the new designation to replace the old one: ";
			
			cin.getline(new_des,30);
			check_if_list_is_empty_case_7 = updateDesignation(listPtr, old_des, new_des);
			if (check_if_list_is_empty_case_7 == false)
				cout << "The list is empty" << endl;
			system("pause");
			break;
		case 8:
			int year;
			bool check_if_list_is_empty_case_8;
			List *displayList;
			displayList = new List;
			cout << "Input the year you want to search from: " << endl;
			cin >> year;
			check_if_list_is_empty_case_8 = filterByYear(listPtr, year, displayList);
			if (check_if_list_is_empty_case_8 == false)
				cout << "There is no matching year. " << endl;
			else if (check_if_list_is_empty_case_8 == true)
				printEmployeeList(displayList->headPtr);
			system("pause");
			break;
		default:
			break;
	...
	} while (choice != 9); //when choice = 7, the loop stop and the program stops.

	return 0;

}

bool updateDesignation(List *list1, char *old_des, char *new_des) //Done
{
	bool return_value = false;
	EMPLOYEE *employeePtr = new EMPLOYEE;
	employeePtr = list1->headPtr;
	while (employeePtr != NULL)
	{
		if (strcmp(employeePtr->data.designation,old_des)==0) // Compare the two string
		{
			strcpy_s(employeePtr->data.designation, new_des); //Replace the old name with the new name
			employeePtr = employeePtr->nextPtr;
			return_value = true;
		}
		else
			employeePtr = employeePtr->nextPtr;
	}
	if (list1 == NULL) //If nothing is in the list
		return_value = false;
	printEmployeeList(list1->headPtr); //Print the edited list
...
bool filterByYear(List *list1, int year, List *list2)
{
	...
	employeePtr = list1->headPtr;
	int year_join;
	year_join = (year % 2000) * 100; //Make the last two digit revelant
	while (employeePtr != NULL)
	{
		if (employeePtr->data.id % year_join < 100) //If modolus less than 100, means it is the year I'm looking for 
		{
			insertEmployeeRecord(list2, employeePtr->data, list2->count);
			employeePtr = employeePtr->nextPtr;
			return_value = true;
		}
		else
	...
	if (list1 == NULL || list2 == NULL)
		return return_value = false; //If either one is nothing, return 0
	return return_value;
}